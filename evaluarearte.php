...
<form action="votprocess2.php" method="post">
<table>
<?php
	$pa=1;
 	$s=mysql_query("SELECT * FROM poster WHERE art='$pa' AND ses='$ses'");
	while($k=mysql_fetch_array($s))
	{
	?>
	<tr>
...
{
?>
<form action="voting22.php" method="post">
...
$s=mysql_query("SELECT * FROM poster AND ses='$ses'");
while($k=mysql_fetch_array($s))
{
if($k['id']==$idact)
{
